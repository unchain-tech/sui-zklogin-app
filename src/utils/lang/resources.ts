export const resources = {
  en: {
    translation: {
      ...
      "62a0a307":
        "The ephemeral key pair is used to sign the <1>TransactionBlock</1>",
      ...
      "2397bcd8":
        "(Generated through<1>ephemeralKeyPair</1> <2>maxEpoch</2> \n" +
        "      <3>randomness</3>)",
      ...
      c4a666f0:
        "<1>Where to Save:</1><2>1. Ask the user to remember (send to user's email)</2><3>2. Store on the client side (browser)</3><3>3. Save in the APP Backend database, corresponding one-to-one with UID</3>",
      "2fb333f5": "Step 5: Generate User's Sui Address",
      e05797f4:
        "The user's Sui address is determined by <1>sub</1>,<1>iss</1>,<1>aud</1> and <1>user_salt</1> together. For the same JWT, <1>sub</1>, <1>iss</1>, and <1>aud</1> will not change with each login.",
  ...
  zh: {
    translation: {
      ...
      "2397bcd8":
        "（通过 <1>ephemeralKeyPair</1> <2>maxEpoch</2> \n      <3>randomness</3> 生成）",
      ...
      "71c429d2": "生成扩展的临时公钥",
      "16ebd660":
        "使用生成临时的公钥扩展(extendedEphemeralPublicKey)来生成ZK Proof，SUI 提供了一个 backend service（也可以 Run 一个 Docker）",
      "33893c96": "获取ZK Proof",
